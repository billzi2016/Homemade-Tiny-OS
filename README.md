...
这个项目里的“磁盘”不是宿主机真实分区，而是 Tiny OS 自己管理的镜像文件。

默认情况下：

- 手动启动示例会在项目根目录使用 `tinyos.disk`
- `scripts/` 演示脚本会把磁盘写到 `scripts/.artifacts/`
- `tests/` 自动化测试会把磁盘写到 `tests/.artifacts/`
...
也就是说，虚拟磁盘默认保留在本地调试环境中，不会被误提交到仓库。
...
请直接看 [QUICKSTART.md](QUICKSTART.md) 里的“虚拟磁盘位置与清理”一节。
